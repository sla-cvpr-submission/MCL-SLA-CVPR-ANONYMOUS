...
class Imagelists_OfficeHome_Repeat(TorchData.Dataset):
    def __init__(self, image_list, data_arr, index_dict, root="./data/multi/",
                 transform=None, target_transform=None, test=False, domain_ind=None, seed=None):
        super().__init__()
        imgs, labels = make_dataset_fromlist(image_list)
...
def build_dataset_officehome(args):
    ...
    # load np array
    data_arr = np.load('/home/xxx/SLC/office_home_data/office_home_image.npy')
    with open('/home/xxx/SLC/office_home_data/office_home_index.pkl', 'rb') as f:
    ...
    target_dataset = Imagelists_OfficeHome_Repeat(image_set_file_t, data_arr, index_dict, root=root,
                                                  transform=data_transforms[args.labeled_transform], seed=args.seed)
    ...
    drop_last = True
    bs = args.bs
    source_loader = TorchData.DataLoader(source_dataset, sampler=train_sampler(source_dataset, generator=get_generator(args.seed)), worker_init_fn=seed_worker,
    ...
    target_loader_unl = TorchData.DataLoader(target_dataset_unl, sampler=train_sampler(target_dataset_unl, generator=get_generator(args.seed)), worker_init_fn=seed_worker,
